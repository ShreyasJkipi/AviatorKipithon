...
col2.write(":cricket_bat_and_ball: **KIPL | GUJRAT MACHINE GLADIATORS** ", font="Roboto 24")
...
def main():
    ...
    with tabs[0]:
        ...
        prob_stat = """+ The primary challenge for our online subscription-based firm is customer churn, where subscribers terminate their subscriptions, directly impacting revenue and growth potential."""
        st.markdown(prob_stat)

        st.markdown("""+ Understanding the factors driving churn and implementing strategies to reduce it are critical for maintaining business stability and fostering long-term growth.""")
        ...
        st.markdown("""+ Improved Customer Satisfaction: Proactively addressing potential churn issues can enhance overall customer satisfaction by demonstrating responsiveness to customer needs and preferences.""")

        
        st.markdown("""+ Enhanced Marketing Efficiency: Segmenting customers based on churn probability allows for more effective allocation of marketing resources, ensuring that efforts are focused on retaining high-value customers. """)
        
        st.markdown("""+ Competitive Advantage: Utilizing churn prediction analysis can differentiate our business by demonstrating our commitment to customer-centric strategies, potentially attracting and retaining more customers. """)
        ...
        c2.image("https://miro.medium.com/v2/resize:fit:720/format:webp/1*Dvx1j18vyKyvLlIpxzVSmQ.png")
    ...
    with tabs[1]:
            ...
            df= session.table('CUST_CHURN.CUSTOMER.CHURN_DATA_CLEAN').to_pandas()
        ...
        # Display DataFrame with customized column names and size
            ...
            if st.checkbox("Univariate Analysis"):
                ...
                df = df[df.CHURN==1]
                x = st.selectbox("Select Category to view distribution", ['TENURE', 'PREFERREDLOGINDEVICE', 'CITYTIER', 'WAREHOUSETOHOME', 
                ...
                if x=='PREFEREDORDERCAT':
                    top_df = top_counts(df,'PREFEREDORDERCAT',9)
                    fig = px.pie(top_df, values='Count', names=x, hole = 0.3)
                    fig.update_layout(title={'text':f"{x} Distribution", 'x': 0.5, 'y':0.95})
                    st.plotly_chart(fig, use_container_width=True)

                if x=='MARITALSTATUS':
                    top_df = top_counts(df,'MARITALSTATUS',9)
                    fig = px.pie(top_df, values='Count', names=x, hole = 0.3)
                    fig.update_layout(title={'text':f"{x} Distribution", 'x': 0.5, 'y':0.95})
                    st.plotly_chart(fig, use_container_width=True)

                if x=='COMPLAIN':
                    top_df = top_counts(df,'COMPLAIN',9)
                    fig = px.pie(top_df, values='Count', names=x, hole = 0.3)
                    fig.update_layout(title={'text':f"{x} Distribution", 'x': 0.5, 'y':0.95})
                    st.plotly_chart(fig, use_container_width=True)
